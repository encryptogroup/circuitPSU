...
void osuCrypto::MPOPRFReceiver::setTimer(Timer &timer) { this->timer = &timer; }

osuCrypto::MPOPRFReceiver::MPOPRFReceiver() {}

void osuCrypto::MPOPRFReceiver::setParams(
    const block &commonSeed, const u64 &set_size, const u64 &logHeight,
    const u64 &width, const u64 &hashLengthInBytes, const u64 &h1LengthInBytes,
...
MPOPRFReceiver::MPOPRFReceiver(const block &commonSeed, const u64 &set_size,
                               const u64 &logHeight, const u64 &width,
                               const u64 &hashLengthInBytes,
                               const u64 &h1LengthInBytes, const u64 &bucket1,
                               const u64 &bucket2)
    : commonSeed(commonSeed), set_size(set_size), logHeight(logHeight),
      width(width), hashLengthInBytes(hashLengthInBytes),
      h1LengthInBytes(h1LengthInBytes), bucket1(bucket1), bucket2(bucket2) {
  height = 1ull << logHeight;
}

u8 *MPOPRFReceiver::run(PRNG &prng, oc::Socket *chls,
                        const std::vector<block> &receiverSet,
                        size_t num_threads) {
  ...
  SilentOtExtSender otExtSender;
  otExtSender.configure(width, 2, num_threads, SilentSecType::SemiHonest);
  cp::sync_wait(otExtSender.genBaseOts(prng, chls[0]));
  std::vector<std::array<block, 2>> otMessages(width);
  cp::sync_wait(otExtSender.send(otMessages, prng, chls[0]));
  ...
  // TODO SET MANUALLY / FIGURE OUT HOW TO DO MULTIPLE THREADS WITH DIFFERENT
  // CHANNELS
  ...
  auto routine1 = [&](size_t tid) {
    ...
    aes.ecbEncBlocks(aesInput.data() + start_idx, end_idx - start_idx,
                     aesOutput.data() + start_idx);
  ...
  auto routine2 = [&](size_t tid) {
    ...
    vector<vector<u8>> transLocations(
        widthBucket1, vector<u8>(set_size * locationInBytes + sizeof(u32)));

    for (auto wLeft = start_idx; wLeft < end_idx; wLeft += widthBucket1) {
      auto wRight =
          wLeft + widthBucket1 < end_idx ? wLeft + widthBucket1 : end_idx;
      auto w = wRight - wLeft;
      ...
      for (auto low = 0; low < set_size; low += bucket1) {
        ...
        aes.ecbEncBlocks(recvSet.data() + low, up - low,
                         randomLocations.data());
        for (u64 i = 0; i < w; ++i) {
          for (u64 j = low; j < up; ++j) {
            memcpy(transLocations[i].data() + j * locationInBytes,
                   (u8 *)(randomLocations.data() + (j - low)) +
                       i * locationInBytes,
                   locationInBytes);
          }
        }
      ...
      for (u64 i = 0; i < w; ++i) {
        for (u64 j = 0; j < set_size; ++j) {
          auto location =
              (*(u32 *)(transLocations[i].data() + j * locationInBytes)) &
              shift;
      ...
      for (auto i = 0; i < w; ++i) {
        ...
        for (auto j = 0; j < heightInBytes; ++j) {
          sentMatrix[i * heightInBytes + j] ^=
              matrixA[i][j] ^ matrixDelta[i][j];
        }
      }
      cp::sync_wait(chls[tid].send(std::move(sentMatrix)));
