...
void osuCrypto::MPOPRFSender::setTimer(Timer &timer) { this->timer = &timer; }

void osuCrypto::MPOPRFSender::run(PRNG &prng, Socket *chls,
                                  size_t num_threads) {
  ...
  SilentOtExtReceiver otExtReceiver;
  otExtReceiver.configure(width, 2, num_threads, SilentSecType::SemiHonest);
  cp::sync_wait(otExtReceiver.genBaseOts(prng, chls[0]));
  BitVector choices(width);
  std::vector<block> otMessages(width);
  prng.get(choices.data(), choices.sizeBytes());
  cp::sync_wait(otExtReceiver.receive(choices, otMessages, prng, chls[0]));
  ////////////// Initialization //////////////////////
  ...
  auto routine = [&](size_t tid) {
    ...
    for (auto wLeft = start_idx; wLeft < end_idx; wLeft += widthBucket1) {
      ...
      cp::sync_wait(chls[tid].recv(recvMatrix));
      for (auto i = 0; i < w; ++i) {
...
void osuCrypto::MPOPRFSender::setParams(
...
osuCrypto::MPOPRFSender::MPOPRFSender(const block &commonSeed,
                                      const u64 &set_size, const u64 &logHeight,
                                      const u64 &width,
...
vector<u8> osuCrypto::MPOPRFSender::get_oprf(std::vector<block> &senderSet) {
  ...
  for (auto wLeft = 0; wLeft < width; wLeft += widthBucket1) {
    ...
    for (auto low = 0; low < set_size; low += bucket1) {
      ...
      commonAes.ecbEncBlocks(sendSet.data() + low, up - low,
    ...
    for (u64 i = 0; i < w; ++i) {
      for (u64 j = 0; j < set_size; ++j) {
        auto location =
            (*(u32 *)(transLocations[i].data() + j * locationInBytes)) & shift;
        transHashInputs[i + wLeft][j >> 3] |=
            (u8)((bool)(matrixC[i + wLeft][location >> 3] &
                        (1 << (location & 7))))
            << (j & 7);
      }
    }
  ...
  for (auto low = 0; low < set_size; low += bucket2) {
    ...
    for (auto j = low; j < up; ++j) {
      ...
      H.Final(hashOutput);
      memcpy(results.data() + j * hashLengthInBytes, hashOutput,
             hashLengthInBytes);
    }
  }
  return results;
