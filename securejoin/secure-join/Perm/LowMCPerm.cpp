...
namespace secJoin
{
	...
	macoro::task<> LowMCPermSender::generate(Perm perm, PRNG& prng, coproto::Socket& chl, PermCorSender& dst)
	{
		...
		for (u64 i = 0; i < mN; ++i)
		{
			secJoin::copyBytesMin(dst.mDelta[i], d.subspan(i * blocksPerRow, blocksPerRow));
		}
	}
	...
	macoro::task<> LowMCPermReceiver::generate(PRNG& prng, coproto::Socket& chl, PermCorReceiver& dst)
	{
		...
		for (u64 i = 0; i < mN; ++i)
		{
			secJoin::copyBytesMin(dst.mB[i], b.subspan(i * blocksPerRow, blocksPerRow));
		}
		...
		for (u64 i = 0; i < mN; i++)
		{
			...
			secJoin::copyBytesMin(dst.mA[i], a);
		}
