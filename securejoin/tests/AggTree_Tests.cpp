    ...
    for (u64 t = 0; t < numTrials; ++t)
    {
        ...
        for (u64 i = 0; i < numInputs; ++i)
        {
            ...
            for (u64 j = 0; j < numShares; ++j)
            {
                ...
                secJoin::copyBytes(in[j], inputs[j][i]);
            }
            ...
            for (auto level : { 0,1 })
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        secJoin::copyBytes(expS.subMatrix(mR, mN - mN0), s.subMatrix(mN0));
                        secJoin::copyBytes(expPreC.subMatrix(mR, mN - mN0), c.subMatrix(mN0));
