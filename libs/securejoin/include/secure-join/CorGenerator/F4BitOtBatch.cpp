...
namespace secJoin
{
	...
	void F4BitOtBatch::RecvBatch::mock(u64 batchIdx, u64 n)
	{
		...
		if (1)
		{
			secJoin::setBytes(mOts, 0);
			secJoin::setBytes(mChoiceLsb, 0);
			secJoin::setBytes(mChoiceMsb, 0);
			return;
		}
		...
		span<oc::block> A)
	{
		...
		secJoin::setBytes(shuffle, static_cast<u8>(1 << 7));
		for (u64 i = 0; i < 16; ++i)
		...
		span<oc::block> B)
	{
		...
		auto shuffle = std::array<block, 16>{};
		secJoin::setBytes(shuffle, static_cast<char>(1 << 7));
		for (u64 i = 0; i < 16; ++i)
			shuffle[i].set<u8>(i, 0);
